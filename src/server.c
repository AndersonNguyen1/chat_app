	...
	int DataSocketFD)
{
	...
	else if(0 == strncmp(RecvBuf, "DL", 2)) {
	    if(delete_account(RecvBuf)) {
	            strncpy(SendBuf, "You deleted your account", sizeof(SendBuf)-1);
	    }
	    else {
		    strncpy(SendBuf, "You were unable to delete your account", sizeof(SendBuf) -1);
	    }
	    SendBuf[sizeof(SendBuf)-1] = 0;
	}
...
/*			FD_SET(clients[i].SocketFD,&WriteFD);*/
#ifdef DEBUG
